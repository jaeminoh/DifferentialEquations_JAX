...
class sno:
    ...
    def __init__(self):
        ...
        self.init_b, self.apply_b = MLP(layers_b, np.sin, w0=8.0)
        layers_t = [1, 100, 100, 200]
        self.init_t, self.apply_t = MLP(layers_t, np.sin, w0=10.0)
        self.domain_te = (
            np.linspace(*self.T, N_te),
            np.linspace(*self.X, N_te),
            np.linspace(*self.a, N_te),
            np.linspace(*self.b, N_te),
        )
        self.domain_tr = [
            np.linspace(*self.T, N_tr),
            np.linspace(*self.X, N_tr),
            np.linspace(*self.a, N_tr),
            np.linspace(*self.b, N_tr),
        ]
        self.u0 = vmap(vmap(self._u0, (0, None), 0), (None, 0), 1)
    ...
    def pde(self, params, t, x, a, b, is_sampling=False):
    ...
    def loss(self, params, t, x, a, b):
        loss = (
            self.pde(params, t, x, a, b, is_sampling=False).mean()
    ...
    def train(self, optimizer, nIter, seed):
        ...
        state = optimizer.init_state(params, *self.domain_tr)
        loss_log = []
        for epoch in (pbar := trange(nIter)):
            ...
            if epoch % 100 == 0:
                ...
                pde_loss_t, pde_loss_x, pde_loss_a, pde_loss_b = self.pde(
                    params, *self.domain_tr
                )
                train_key, sample_key = random.split(train_key)
                self.domain_tr[0] = sampling(
                    sample_key, self.domain_tr[0], pde_loss_t, *self.T
                )
                self.domain_tr[1] = sampling(
                    sample_key, self.domain_tr[1], pde_loss_x, *self.X
                )
                self.domain_tr[2] = sampling(
                    sample_key, self.domain_tr[2], pde_loss_a, *self.a
                )
                self.domain_tr[3] = sampling(
                    sample_key, self.domain_tr[3], pde_loss_b, *self.b
                )
...
truth_v = vmap(
    vmap(
        vmap(vmap(truth, (0, None, None, None), 0), (None, 0, None, None), 1),
        (None, None, 0, None),
        2,
    ),
    (None, None, None, 0),
...
u_true = truth_v(*model.domain_te)
...
l2_err = vmap(vmap(relative_l2, (2, 2), 0), (3, 3), 1)(u, u_true)
...
im = ax3.imshow(
    l2_err.T, origin="lower", extent=[*model.a, *model.b], aspect="auto", cmap="jet"
)
