...
from jax.numpy.fft import rfft, irfft, rfftfreq, fft, ifft, fftfreq
...
class vlasov_poisson:
    ...
    def eqn(self, t, fhat):
        ...
        fhat = fhat.reshape(
            (self.Nx, -1)
        )  # this may be omitted by using the "tree_math" package.
    ...
    def solve(self):
        ...
        solver = RK45(self.eqn, 0.0, f0hat, self.T)
...
class kuramoto_sivashinsky:
    ...
    def __init__(
        ...
        self.X = X
        self.Nx = Nx
        self.xx = np.linspace(0.0, X, Nx, endpoint=False)
    ...
    def solve(self):
        u0 = self.u0(self.xx)
        u0hat = rfft(u0)
        solver = RK45(self.eqn, 0.0, u0hat, self.T, max_step=self.dt)
        uhat_list = [u0hat]
        while solver.status == "running":
            solver.step()
...
if __name__ == "__main__":
    ...
    plt.imshow(
        u.T, origin="lower", aspect="auto", cmap="jet", extent=[0, model.T, model.X, 0]
    )
    plt.title(f"time: {t}")
    plt.colorbar()
    plt.tight_layout()
